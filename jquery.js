...
fonction testContext (contexte) {
	renvoie le contexte && typeof context.getElementsByTagName! == "non défini" && context;
}
...
setDocument = Sizzle.setDocument = fonction (nœud) {
	...
	// Vérifiez si getElementsByTagName ("*") ne renvoie que des éléments
	...
	// Vérifier si getElementById renvoie des éléments par leur nom
	// Les méthodes getElementById cassées ne récupèrent pas les noms définis par programme,
	// utilisez donc un test getElementsByName du rond-point
		...
		Expr.find ["ID"] = fonction (id, contexte) {
			if (typeof context.getElementById! == "undefined" && documentIsHTML) {
				...
				if (elem) {
					...
					// Se replier sur getElementsByName
	...
	Expr.find ["TAG"] = support.getElementsByTagName?
		fonction (balise, contexte) {
			...
			// Les nœuds DocumentFragment n'ont pas gEBTN
			} else if (support.qsa) {
	...
	// Prise en charge de QSA et matchesSelector

	// matchesSelector (: active) signale false quand true (IE9 / Opera 11.5)
	rbuggyMatches = [];

	// qSa (: focus) signale faux lorsqu'il est vrai (Chrome 21)
	...
	if ((support.qsa = rnative.test (document.querySelectorAll))) {
		...
		assert (fonction (el) {
			...
			// Échec du `sélecteur # id du sélecteur combinateur-frère sur la page '
		...
		assert (fonction (el) {
			...
			// Prise en charge: Windows 8 Native Apps
			...
			// Prise en charge: IE8
			// Appliquer la respect de la casse de l'attribut name
			if (el.querySelectorAll ("[name = d]"). length) {
				rbuggyQSA.push ("nom" + espaces blancs + "* [* ^ $ |! ~]? =");
			}
			...
			// IE: le sélecteur désactivé ne récupère pas les enfants des ensembles de champs désactivés
	...
	if ((support.matchesSelector = rnative.test ((matches = docElem.matches ||
		...
		assert (fonction (el) {
			// Vérifiez s'il est possible de faire des correspondances
	...
	contient = hasCompare || rnative.test (docElem.contains)?
		fonction (a, b) {
			var adown = a.nodeType === 9? a.documentElement: a,
				bup = b && b.parentNode;
			retourner un === bup || !! (bup && bup.nodeType === 1 && (
				adown.contient?
					adown.contains (bup):
	...
	fonction (a, b) {
		...
		if (comparer & 1 ||
			...
			if (un === document || a.ownerDocument === preferDoc && contient (preferDoc, a)) {
				return -1;
			}
			if (b === document || b.ownerDocument === preferDoc && contient (preferDoc, b)) {
				retour 1;
			}
	...
	fonction (a, b) {
		...
		// Si les nœuds sont frères et sœurs, nous pouvons faire une vérification rapide
		} else if (aup === bup) {
			retourner siblingCheck (a, b);
		}
		...
		// Descendez dans l'arbre à la recherche d'un écart
		while (ap [i] === bp [i]) {
			i ++;
		}

		retour je?
			// Effectue une vérification frère si les nœuds ont un ancêtre commun
			siblingCheck (ap [i], bp [i]):

			// Sinon, les nœuds de notre document trient d'abord
			ap [i] === preferDoc? -1 :
			bp [i] === preferDoc? 1 :
			0;
	};
...
Sizzle.matchesSelector = fonction (elem, expr) {
	// Définissez les variables de document si nécessaire
	...
	if (support.matchesSelector && documentIsHTML &&
		...
		essayez {
			...
			if (ret || support.disconnectedMatch ||
					// De plus, les nœuds déconnectés sont censés se trouver dans un document
					// fragment dans IE 9
...
Sizzle.contains = fonction (contexte, elem) {
	// Définissez les variables de document si nécessaire
	if ((context.ownerDocument || context)! == document) {
		setDocument (contexte);
	}
	return contient (context, elem);
};

Sizzle.attr = fonction (elem, nom) {
	// Définissez les variables de document si nécessaire
	if ((elem.ownerDocument || elem)! == document) {
		setDocument (elem);
	}
...
Sizzle.uniqueSort = fonction (résultats) {
	...
	// Sauf si nous * savons * nous pouvons détecter les doublons, supposer leur présence
...
getText = Sizzle.getText = function (elem) {
	...
	// Ne pas inclure de noeuds d'instructions de commentaire ou de traitement
...
Expr = Sizzle.selectors = {
	...
	preFilter: {
		...
		"ENFANT": fonction (match) {
			/ * correspond à matchExpr ["CHILD"]
				...
				4 Composante xn de l'argument xn + y ([+ -]? \ D * n |)
				5 signe de composant xn
				6 x composants xn
			...
			* /
			...
			if (match [1] .slice (0, 3) === "nth") {
				...
				// rappelez-vous que la fausse / vraie distribution respectivement à 0/1
		...
		"PSEUDO": fonction (correspondance) {
			...
			// Supprime les caractères en excès des arguments non cotés
				...
				// Obtenir un excès de tokenize (récursivement)
	...
	filtre: {
		...
		"CLASS": fonction (className) {
			var pattern = classCache [className + ""];

			modèle de retour ||
				(pattern = new RegExp ("(^ |" + espaces blancs + ")" + className + "(" + espaces blancs + "| $)")) &&
				classCache (className, fonction (elem) {
		...
		"ATTR": fonction (nom, opérateur, chèque) {
			fonction de retour (elem) {
				...
				opérateur de retour === "="? résultat === contrôle:
					opérateur === "! ="? result! == check:
					...
					opérateur === "| ="? result === check || result.slice (0, check.length + 1) === check + "-":
					faux;
			};
		},
			...
			retourner d'abord === 1 && dernier === 0?
				...
				fonction (elem, context, xml) {
					...
					si (parent) {
						...
						//: (premier | dernier | uniquement) - (enfant | de type)
						si (simple) {
							while (dir) {
								node = elem;
								while ((node ??= node [dir])) {
								...
								// Sens inverse pour: seulement- * (si nous ne l'avons pas encore fait)
						...
						if (forward && useCache) {
							...
							// Défense contre les propriétés clonées (jQuery gh-1709)
							uniqueCache = externalCache [node.uniqueID] ||
								(cache externe [node.uniqueID] = {});
							...
							while ((node ??= ++ nodeIndex && node && node [dir] ||
								...
								// Retour à la recherche d'elem depuis le début
							...
							// Utiliser l'index des éléments précédemment mis en cache si disponible
							if (useCache) {
								// ... d'une manière compatible avec gzip
								node = elem;
								externalCache = node [expando] || (nœud [expando] = {});
							...
							// xml: nième enfant (...)
							// ou: nième-dernier-enfant (...) ou: nième (-dernier)? - de type (...)
							if (diff === false) {
						...
						// Incorporez l'offset, puis vérifiez la taille du cycle
						diff - = dernier;
						return diff === first || (diff% en premier === 0 && diff / en premier> = 0);
					}
				};
		},

		"PSEUDO": fonction (pseudo, argument) {
			// les noms de pseudo-classes ne respectent pas la casse
			// http://www.w3.org/TR/selectors/#pseudo-classes
			// Prioriser par sensibilité à la casse dans le cas où des pseudos personnalisés sont ajoutés avec des lettres majuscules
	...
	pseudos: {
		...
		"not": markFunction (fonction (sélecteur) {
			// Découpe le sélecteur passé pour compiler
			...
			retour matcher [expando]?
				markFunction (fonction (graine, correspondances, contexte, xml) {
					var elem,
						...
						i = longueur de semence;

					// Correspond à des éléments inégalés par `matcher`
					alors que je-- ) {
						if ((elem = inégalé [i])) {
		...
		"has": markFunction (function (sélecteur) {
			fonction de retour (elem) {
				retour Sizzle (sélecteur, elem) .longueur> 0;
			};
		}),

		"contient": markFunction (fonction (texte) {
		...
		// "Indique si un élément est représenté par un sélecteur a: lang ()
		...
		"vérifié": fonction (elem) {
			// En CSS3,: vérifié doit renvoyer les éléments vérifiés et sélectionnés
			// http://www.w3.org/TR/2011/REC-css3-selectors-20110929/#checked
			var nodeName = elem.nodeName.toLowerCase ();
			return (nodeName === "input" && !! elem.checked) || (nodeName === "option" && !! elem.selected);
		},

		"sélectionné": fonction (elem) {
		...
		"vide": fonction (elem) {
			...
			//: vide est annulé par l'élément (1) ou les nœuds de contenu (texte: 3; cdata: 4; entité ref: 5),
		...
		"texte": fonction (elem) {
			...
			return elem.nodeName.toLowerCase () === "input" &&
				...
				// Prise en charge: IE <8
				// Les nouvelles valeurs d'attribut HTML5 (par exemple, "recherche") apparaissent avec elem.type === "texte"
		...
		"last": createPositionalPseudo (fonction (matchIndexes, longueur) {
			return [longueur - 1];
		}),

		"eq": createPositionalPseudo (fonction (matchIndexes, longueur, argument) {
			retourner [argument <0? argument + longueur: argument];
		}),

		"even": createPositionalPseudo (fonction (matchIndexes, longueur) {
			var i = 0;
			pour (; i <longueur; i + = 2) {
				matchIndexes.push (i);
			}
...
tokenize = Sizzle.tokenize = fonction (sélecteur, parseOnly) {
	...
	if (mis en cache) {
		retourner l'analyse uniquement? 0: cached.slice (0);
	}
	...
	while (soFar) {
		...
		if (! matched || (match = rcomma.exec (soFar))) {
			if (match) {
				// Ne consomme pas de virgules de fin comme valides
		...
		if ((match = rcombinators.exec (soFar))) {
			...
			tokens.push ({
				valeur: apparié,
				// Lancer des combinateurs descendants dans l'espace
	...
	// si nous analysons simplement
	// Sinon, lancez une erreur ou renvoyez des jetons
	retourner l'analyse uniquement?
		soFar.length:
		jusque là ?
...
fonction addCombinator (matcher, combinator, base) {
	...
	retourner combinator.first?
		// Vérifier contre l'ancêtre le plus proche / l'élément précédent
		fonction (elem, context, xml) {
			while ((elem = elem [dir])) {
		...
		// Vérifier tous les ancêtres / éléments précédents
		fonction (elem, context, xml) {
			...
			if (xml) {
				...
				while ((elem = elem [dir])) {
					if (elem.nodeType === 1 || checkNonElements) {
						...
						// Prise en charge: IE <9 uniquement
						// Défense contre les propriétés clonées (jQuery gh-1709)
						uniqueCache = externalCache [elem.uniqueID] || (externalCache [elem.uniqueID] = {});
						...
						if (ignorer && ignorer === elem.nodeName.toLowerCase ()) {
							...
							// Attribuer à newCache pour que les résultats se propagent en retour aux éléments précédents
							return (newCache [2] = oldCache [2]);
						} autre {
							// Réutilise newcache pour que les résultats se propagent en retour aux éléments précédents
...
condenser la fonction (inégalée, carte, filtre, contexte, xml) {
	var elem,
		newUnmatched = [],
		i = 0,
...
fonction setMatcher (preFilter, selector, matcher, postFilter, postFinder, postSelector) {
	...
	return markFunction (fonction (graine, résultats, contexte, xml) {
		var temp, i, elem,
			...
			// Préfiltre pour obtenir une entrée de correspondance, préservant une carte pour la synchronisation des résultats de départ
			matcherIn = preFilter && (graine ||! sélecteur)?
				condenser (elems, preMap, preFilter, context, xml):
				elems,

			matcherOut = matcher?
				// Si nous avons un postFinder, ou une graine filtrée, ou un postFiltre non-graine ou des résultats préexistants,
		...
		// Recherche de correspondances primaires
		if (matcher) {
				...
				if (postFinder) {
					...
					alors que je-- ) {
						if ((elem = matcherOut [i])) {
							// Restore matcherIn puisque elem n'est pas encore un match final
...
fonction matcherFromTokens (jetons) {
	var checkContext, matcher, j,
		...
		// La correspondance fondamentale garantit que les éléments sont accessibles à partir de contexte (s) de niveau supérieur
	...
	pour (; i <len; i ++) {
		if ((matcher = Expr.relative [tokens [i] .type]))) {
			...
			// Retour spécial lorsque vous voyez un matcher positionnel
			if (matcher [expando]) {
				// Recherche de l'opérateur relatif suivant (le cas échéant) pour une manipulation correcte
				j = ++ i;
				pour (; j <len; j ++) {
...
function matcherFromGroupMatchers (elementMatchers, setMatchers) {
	var bySet = setMatchers.length> 0,
		...
		superMatcher = fonction (seed, context, xml, results, outermost) {
			var elem, j, matcher,
				matchedCount = 0,
				i = "0",
				inégalée = graine && [],
				...
				// Nous devons toujours avoir soit les éléments de départ, soit le contexte le plus externe
				elems = graine || byElement && Expr.find ["TAG"] ("*", le plus à l'extérieur),
				// Utilisez des dirruns entiers si c'est le matcher le plus externe
			...
			// Tolérer les propriétés NodeList (IE: "longueur"; Safari: <nombre>) correspondant aux éléments par id
			pour (; i! == len && (elem = elems [i])! = null; i ++) {
				...
				// Suivre les éléments inégalés pour les filtres définis
				if (bySet) {
					...
					// Allonge le tableau pour chaque élément, assorti ou non
					if (graine) {
						unmatched.push (elem);
					}
				}
			...
			// Appliquer des filtres d'ensemble à des éléments sans correspondance
			// REMARQUE: ceci peut être ignoré s'il n'y a pas d'éléments sans correspondance (c'est-à-dire `matchedCount`
			// est égal à `i`), sauf si nous n'avons pas visité les éléments _any_ dans la boucle ci-dessus car nous avons
			// pas d'appariement d'éléments et pas de graine.
			// L'incrémentation d'une chaîne initiale "0" `i` permet à` i` de rester une chaîne uniquement dans ce
			// case, qui se traduira par un "00" `matchedCount` qui diffère de` i` mais est également
			// numériquement zéro.
			if (bySet && i! == matchedCount) {
				...
				if (graine) {
					...
					if (matchedCount> 0) {
						alors que je-- ) {
							if (! (inégalé [i] || setMatched [i])) {
								setMatched [i] = pop.call (résultats);
							}
						}
					}

					// Ignore les valeurs d'espace réservé d'index pour obtenir uniquement les correspondances réelles
				...
				// Les correspondances d'ensemble sans pépins réussissant plusieurs apparieurs réussis stipulent le tri
				if (out & out! &&! seed && setMatched.length> 0 &&
			...
			// Ignorer la manipulation des globaux par les matchers imbriqués
			si (le plus à l'extérieur) {
				dirruns = dirrunsUnique;
...
compile = Sizzle.compile = fonction (sélecteur, correspondance / * Usage interne uniquement * /) {
	var i,
		setMatchers = [],
		elementMatchers = [],
...
/ **
 * Une fonction de sélection de bas niveau qui fonctionne avec Sizzle compilé
 * fonctions de sélection
 * @param {String | Function} selector Un sélecteur ou un pré-compilé
 * fonction de sélection intégrée à Sizzle.compile
...
select = Sizzle.select = fonction (sélecteur, contexte, résultats, graine) {
	var i, jetons, jeton, tapez, trouvez,
		compilé = typeof selector === "function" && selector,
	...
	if (match.length === 1) {
		...
		if (tokens.length> 2 && (token = tokens [0]). type === "ID" &&
			...
			// Les matchers précompilés vérifieront toujours l'ascendance, alors augmentez d'un niveau
			} else if (compilé) {
	...
	// Fournit `match` pour éviter la retokenization si nous avons modifié le sélecteur ci-dessus
...
// Toujours supposer les doublons s'ils ne sont pas passés à la fonction de comparaison
support.detectDuplicates = !! hasDuplicate;

// Initialiser par rapport au document par défaut
setDocument ();

// Prise en charge: Webkit <537.32 - Safari 6.0.3 / Chrome 25 (corrigé dans Chrome 27)
// Les nœuds détachés se confondent * les uns les autres *
support.sortDetached = assert (fonction (el) {
	// Doit retourner 1, mais retourne 4 (suivant)
	renvoyer el.compareDocumentPosition (document.createElement ("fieldset")) & 1;
});
...
if (! assert (function (el) {
	...
	addHandle ("type | href | hauteur | largeur", fonction (elem, nom, isXML) {
		si (! isXML) {
...
// Prise en charge: IE <9
// Utilisez defaultValue à la place de getAttribute ("value")
if (! support.attributes ||! assert (function (el) {
	el.innerHTML = "<input />";
	el.firstChild.setAttribute ("valeur", "");
	return el.firstChild.getAttribute ("value") === "";
})) {
	addHandle ("valeur", fonction (elem, nom, isXML) {
...
var dir = fonction (elem, dir, until) {
	var matched = [],
...
fonction winnow (éléments, qualificatif, pas) {
	...
	if (typeof qualifier! == "string") {
		return jQuery.grep (elements, function (elem) {
			return (indexOf.call (qualifier, elem)> -1)! == non;
		});
	}

	// Filtré directement pour les sélecteurs simples et complexes
	return jQuery.filter (qualificatif, éléments, pas);
}
...
jQuery.fn.extend ({
	find: fonction (sélecteur) {
		...
		if (typeof selector! == "string") {
			retourner this.pushStack (jQuery (sélecteur) .filter (function () {
				pour (i = 0; i <len; i ++) {
		...
		retour len> 1? jQuery.uniqueSort (ret): ret;
	},
	filtre: fonction (sélecteur) {
		renvoie this.pushStack (winnow (this, selector || [], false));
	},
	pas: fonction (sélecteur) {
		renvoie this.pushStack (winnow (this, selector || [], true));
	},
